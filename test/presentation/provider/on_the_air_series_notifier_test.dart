...
import 'on_the_air_series_notifier_test.mocks.dart';

@GenerateMocks([GetOnTheAirSeries])
void main() {
  late MockGetOnTheAirSeries mockGetOnTheAirSeries;
  late OnTheAirSeriesNotifier notifier;
  late int listenerCallCount;

  setUp(() {
    listenerCallCount = 0;
    mockGetOnTheAirSeries = MockGetOnTheAirSeries();
    notifier = OnTheAirSeriesNotifier(getOnTheAirSeries: mockGetOnTheAirSeries)
      ..addListener(() {
        listenerCallCount++;
      });
  });
  ...
  test('should change series data when data is gotten successfully', () async {
    ...
    when(mockGetOnTheAirSeries.execute())
        .thenAnswer((_) async => Right(tSeriesList));
    // act
    await notifier.fetchOnTheAirSeries();
    // assert
